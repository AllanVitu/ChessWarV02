...
const handleApiProtocolError = (event: Event) => {
  ...
  notifyError(
    "Service API",
    `Reponse invalide sur ${path}. Verifiez le rewrite Apache /api.`,
  );
};
...
onMounted(() => {
  ...
  window.addEventListener(
    API_PROTOCOL_ERROR_EVENT,
    handleApiProtocolError as EventListener,
  );
...
<template>
  ...
  <div
    ...
    class="offline-banner offline-banner--pending"
    role="status"
  >
    <div>
      <p class="offline-banner__title">Actions en attente</p>
      <p class="offline-banner__text">
        {{ pendingCount }} action{{ pendingCount > 1 ? "s" : "" }} en cours de
        synchronisation.
      </p>
    </div>
    <button class="button-ghost" type="button" @click="flushQueue">
      Synchroniser
    </button>
  </div>
